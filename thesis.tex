...
%==============================================================
    ...
    built from the following ingredients :
    \begin{itemize}
        \item the gauge symmetries corresponding to the interactions ;
        \item the fermionic fields and their properties ;
        \item the scalar field leading to spontanneous symmetry breaking.
    \end{itemize}

    The Standard Model includes three interactions : electromagnetic, weak and strong.
    ...
    three families called generation according to their masses.
    ...
    the strong and weak interaction was short-range whereas the electromagnetic
    interaction is infinite-range. It got understood later that the strong interaction
    appears short-range because of confinement. However, attempting to describe the
    weak interaction with an $SU(2)_L$ gauge symmetry proved unsuccessful as mass terms
    for the gauge bosons are breaking the symmetry and therefore not allowed.

    Nevertheless, a remarkable property of certain physical system is how symmetry can
    be spontanneously broken. A straightforward experience is to place a pen perpendicular
    ...
    outcome can be realized breaks the cylindrical symmetry of the system.

    It was discovered that this idea can be succesfully applied to the electroweak symmetry
    $U(1)_Y \times SU(2)_L$ to obtain massive gauge bosons and massive fermions, via a
    spontanneous breaking of this symmetry first introduced by Robert Brout, François
    Englert and Peter Higgs.

    We onsider the introduction of a complex scalar $SU(2)$ doublet $\phi$ with $L = 1$,
    and $Y = 1$,
    \eq{higgsDoublet}
    {
        \phi
        ...
        =
        \frac{1}{\sqrt{2}} \begin{pmatrix} h^+ \cdot e^{i\theta^+} \\ h^0 \cdot e^{i\theta^0} \end{pmatrix}.
    }
    ...
    values of $x^\mu$ such that $h^+ = \theta^+ = \theta^0 = 0$ and $h^0(x^\mu) = v + h(x^\mu)$
    with $h(x^\mu) = 0$ in vacuum. This is called the unitary jauge in which $\phi$ takes the form
    ...
    start from the covariant derivative
    \eqalign{CovariantDerivativeSU2U1}
    {
        D_\mu
        & =
        \partial_\mu - i g_B B_\mu - i g_W t_a W^a_\mu \nonumber\\
        & =
        \partial_\mu - i
        \begin{pmatrix}
    ...
    \eqalign{DefinitionAZW}
    {
        ...
        W_\mu  & = W_\mu^1 + i  W_\mu^2
    }
    Expanding the Lagrangian of $\phi$, one may find
    \eqalign{HiggsFieldKinetic}
    {
    D_\mu \phi D^\mu \phi & = \underbrace{\frac{1}{2}\partial_\mu \phi \partial^\mu \phi}_{h \text{ kinetic}}
                            + \underbrace{(\frac{v^2 g_W^2}{2}) W_\mu W^\mu}_{W \text{ mass}}
                            + \underbrace{(\frac{v^2 g_{WB}^2}{2}) Z_\mu Z^\mu}_{Z \text{ mass}}\nonumber\\
