...
%==============================================================
    ...
    %==============================================================
        ...
        \subsubsection{Motivation and constraints from the Higgs sector}
            ...
            boson can be light, via the boson-fermion symmetry which that protects the mass
            of the Higgs from diverging because of its corrections. This is illustrated on
            ...
            to each fermion (one for each chirality) so that the corrections to cancel each others.
            ...
            One can study the level of tunning needed as function of the mass of the superpartner
            of the top, the stop, for the theory to keep providing a natural explanation to the hierarchy
            ...
            for SUSY to remain natural \refNeededd. This makes the search for stops an important channel
            for possible SUSY discovery at the LHC.

            In addition to the argument of naturalness, one may derive constraints on the stop
            sector directly from the knowledge of the Higgs mass \todo{arXiv:1212.6847}. In
            ...
            where $X_t \definedAs (A_t + \mu \text{cot} ) \beta) / \mass{S}$ is the stop mixing
            ...
            function of $X_t$, as shown on the left of \reffig{fig:stopSectorConstraints} when assuming $tan \beta
        ...
        \insertTwoFigures{stopSectorConstraints}
                         ...
                         red area is forbidden because, as seen on the right plot, $m_S$
                         should be higher than about 600\GeV if $\mass{h} = 125\GeV$ ;
            ...
            as described in \todo{arXiv:1212.6847}. As the photon is not massive,
            ...
            the lines in the right plot of \reffig{fig:stopSectorConstraints} plot illustrates
            how a $\Delta_t$ value would constrain the $(\mass{\tilde{t}_1},\mass{\tilde{t}_2})$
            space.
        ...
        candidate. This happens in particular in R-parity conserving models, where the
        ...
        relation to the electroweak sector makes it the perfect WIMP candidate.
        ...
        Nevertheless, the sneutrino might be motivated as well in models attempting to
        explain the mass of the neutrinos, while the gravitino as LSP appears in gravity *
        mediated SUSY breaking (GMSB) models.
        ...
        $1\TeV$, together with the case for a dark matter candidate, such as the lightest
        neutralino $\lneutralino$ being the LSP. While we could simply concentrate on these
        ...
        than the stop $\lstop$. The process targetted is the direct production of stop
        quark pairs $\lstop \lstop$ at the LHC.
...
$\lstop \rightarrow b \lchargino$ (0.5/450/50) & 248 $\pm$ 16        & 167 $\pm$ 14  & 146 $\pm$ 13  \\
    ...
    \subsection{Discriminating variables \label{sec:analysis_variables}}
        ...
        taken with $(\mass{\lstop},\mass{\lneutralino}) = (175,50)\GeV$ % FIXME todo

        \insertFigure{variables/leadingNonBPtN5}
                     {0.49}
                     {Stacked plots presenting the distribution of $\pT(\text{leading non }b\text{-jet})$
                     with the same conditions of \reffig{fig:variables/METandMEToverSqrtHT},
                     except that the signal benchmarks used are with $(\mass{\lstop},
                     \mass{\lneutralino}) = (175,50)\GeV$, i.e. in the $\deltam$ region where
                     this variable is expected to be discriminant due to
                     the presence of an ISR.} % FIXME todo
   ...
   \end{equation}
    ...
    \insertFigure{FOMcomparison}
                 {0.5}
                 {Comparison \todo{arXiv:1007:1727} of the significance (denoted by med[$Z_0$|1])
...
\end{table}
    ...
    Without applying any signal region cuts, the propotion of $\oneLeptonTop$ in this
    region is about 85\%.
...
\end{table}
    ...
    decay mode considered (i.e. $\lstop \rightarrow t \lneutralino$ or $\lstop \rightarrow
    b \lchargino$ with a given $x$). Comparing the upper limit with the theoretical
    ...
    be built to indirectly probe this scenario \todo{ref to SUS-13-021 and 1407.1043v1}. The
    decay mode $\lstop \rightarrow b \lchargino$ is also challenging in the low $x$ case
    as decay of the two $W$'s are typically softer compared to higher $x$ cases. At low
    $\mass{\lneutralino}$, the mass of the $\lchargino$ is also low and on average, leads
    to less $\MET$ in the event and therefore lower selection efficiency. The final shape
    of this limit is due to concurrence between having a high-enough signal cross-section,
    having an off-shell $W$ to have high-$p_T$ objects to pass the selection, and
    sufficient $\MET$ from the decay of the $\lchargino$.

    The results of this analysis have been combined with an analysis targetting the same
    signals, this time in the dilepton channel. While this channel does not reach a
    comparable performances at high $\deltam$, it is competitive in the low $\deltam$ cases
    due to the lower thresholds on the lepton $\pT$ compared the semileptonic and all hadronic
    ...
    with increasing cuts on this variable. In both the semileptonic and dileptonic analysis,
    the background estimation is mainly data-driven and, provided that the overlap between
    the analysis is close to zero, no correlation between the background systematics are
    ...
    be correlated at 100\%. The results of the analysis are combined and interpreted
    ...
    Futhermore, polarization scenarios can be investigated, directly related to the mixing of
    the stops and the mixing matrices of the neutralinos and charginos introduced in
    \refsec{sec:stopNeutralinoCharginoPheno}. In the $\lstop \rightarrow t \lneutralino$
    decay mode, we investigate two alternative scenarios in the on-shell case depending
    of the handedness of the top from the decay of the stop, either purely lef-handed
    or purely right-handed. As presented on \reffig{fig:resultsCombinedPolarized}, these
    ...
    The maximum increase in the limits reach is found for the scenario with a right-handed
    ...
    \begin{figure}[h!]
        ...
        \rightarrow t \lneutralino$ decay mode with pure lef-handed or right-handed top
    ...
    and the endcap, in which mismeasurement are likely to occur in the tracker and hadronic
    ...
    \subsection{$W$-tagging in the high $\Delta m$ regime}
        ...
        \begin{table}
            ...
            \caption{Yields at preselection level and with an additional cut on $\MT > 120\GeV$,
            using the Monte-Carlo samples for the preparation of the Run II. \label{tab:phys14Preselection}}
        \end{table}
        ...
        \begin{table}
            ...
            \caption{Yields obtain in the signal regions SR1, SR2 and SR3 when considering
       ...
       a luminosity lower than $30\fb$. However, a $2\sigma$ exclusion can happen for
       the (650/325) benchmark between $15$ and $30\fb$ depending of the optimism of the
       scenario. For the (850/100) benchmark, it can also be excluded at the $2\sigma$ level
       with $20\fb$ provided that 50\% of the $\diLeptonTop$ is sucessfully rejected.
