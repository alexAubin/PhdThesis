...
%==============================================================
    ...
    $SU(2)_L \times U(1)_Y$, namely the mass terms of the now physical $W$ and $Z$ bosons.
    However, we may find that the Lagrangian is now invariant under $U(1)_Q$ with
    ...
    introduce terms of the form $E_R\phi^\dagger\Lambda^i_L$, with $i = 1,2,3$. Notice
    how \emph{apriori} there might be mixing between two different generations. All the
    terms for leptons can be written with the form :
    \eqalign{YukawaCoupling}
    {
        \mathcal{L}_{\text{Yukawa}}
        & =
        %- \sum_{i,j} (y^{ij} \bar{L}^i_R \phi^\dagger \Lambda^j_L + y'^{ij} \Lambda^{\dagger j}_L \phi L_R^i)
        - \sum_{i,j} (y^{ij} \bar{L}^i_R \phi^\dagger \Lambda^j_L + \text{h.c.})
        \,\,\,\,\,\,
        \text{with } \Lambda_L = \begin{pmatrix} \Lambda_{+L} \\ \Lambda_{-L} \end{pmatrix}
        \,\,\,\,\,\,
        \text{and } L^i = (E,M,\Theta)
        \nonumber\\
        & =
        %- \sum_{i,j} (v+h)(y^{ij} \bar{L}^i_R \Lambda^j_{-L} + y'^{ij} \bar{\Lambda}^{j}_{-L} L_R^i)\\
        - (v+h)\sum_{i,j} (y^{ij} \bar{L}^i_R \Lambda^j_{-L} + \text{h.c.})\\
        & =
    ...
    phase \todo{introduce it before}. The parameters and consistency of the Standard
